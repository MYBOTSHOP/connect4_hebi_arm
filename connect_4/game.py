...
import sys
sys.path.append("..")
from robot_controller_utils import KThread, Hebi3dofArm, StateMachine
class Board:
# =============================================================================
#     The Board class.
# 
...
# 
#     Args:
#         players(tuple of :class:`Player <connect4.player.Player>`): The two players. The first one is first to play.
...
# =============================================================================
        ...
        self.ssm.arm.initialize_arm()
        thread = KThread(target=self.ssm.arm.handle_joints,)
        thread.start()
        self.ssm.go_home()
        ...
        while winner is None and not self.board.is_full():
            ...
            if current_player ==  self.player2:
                if col == 0:
                    self.ssm.state_transition("C1")
                elif col == 1:
                    self.ssm.state_transition("C2")
                elif col == 2:
                    self.ssm.state_transition("C3")
                elif col == 3:
                    self.ssm.state_transition("C4")
                elif col == 4:
                    self.ssm.state_transition("C5")
                elif col == 5:
                    self.ssm.state_transition("C6")
                elif col == 6:
                    self.ssm.state_transition("C7")
            print('Player {0} plays in column {1}.'.format(
                  current_player, col + 1))
            self.board.insert(col, current_player.coin)
            winner = self.check_winner()
